...
create table Estado (
idEstado int(11) primary key not null,
...
idHuesped int primary key not null auto_increment,
...
delimiter **
create procedure sp_eliminarUsuario(nomUsr nvarchar(30))
begin 
	declare msj nvarchar(30);
    if (select count(*) from Usuario where nombreUsuario = nomUsr and existe = 1) < 1 then
		set msj = "Usuario inexistente";
	else
		update Usuario set existe = 0 where nombreUsuario = nomUsr;
...
begin
	declare msj nvarchar(30);
	if (select count(*) from Huesped where nombreUsuario = nomUsr) > 0 then
		set msj = "Huesped ya existente";
	else
		insert into Huesped
			(nombreUsuario,idMunicipio,nombre,apellidoPaterno,apellidoMaterno,email,numeroTarjeta,telefono,foto,existe)
        values(nomUsr,idMun,nom,pat,mat,mail,numT,tel,img,1);
        set msj = "Huesped creado";
    end if;
    select msj;
end **
delimiter ;
...
begin
	declare msj nvarchar(30);
	if (select count(*) from Huesped where idHuesped = idH and existe = 1) < 1 then
		set msj = "Huesped inexistente";
	else
		update Huesped set 
		nombreUsuario = nomUsr, idMunicipio = idMun, nombre = nom,apellidoPaterno = pat,apellidoMaterno = mat, email = mail, numeroTarjeta = numT, telefono = tel, foto = img
		where idHuesped = idH;
        set msj = "Huesped actualizado";
    end if;
    select msj;
end **
delimiter ;
...
delimiter **
create procedure sp_eliminarHuesped(idH int)
begin
	declare msj nvarchar(30);
	if (select count(*) from Huesped where idHuesped = idH and existe = 1) < 1 then
		set msj = "Huesped inexistente";
	else
		update Huesped set existe = 0 where idHuesped = idH;
        set msj = "Huesped actualizado";
    end if;
    select msj;
end **
...
delimiter **
create procedure sp_consultarHuesped(idH int)
begin
...
delimiter **
create procedure sp_consultarHuespedes()
begin
	select * from Huesped;
end **
delimiter ;
...
begin
	declare msj nvarchar(30);
	if (select count(*) from Cuarto where idCuarto = idC) < 1 then
		set msj = "Cuarto inexistente";
	else
		update Cuarto set 
		  precioDiario = pd, esReservable = esR 
        where idCuarto = idC;
        set msj = "Cuarto actualizado";
    end if;
    select msj;
end **
delimiter ;
...
delimiter **
create procedure sp_ocuparCuarto(enUso boolean, idC int)
begin 
	declare msj nvarchar(30);
    if (select count(*) from Cuarto where idCuarto = idC) < 1 then
		set msj = "Cuarto inexistente";
...
delimiter **
create procedure sp_consultarCuartos()
begin
	select * from Cuarto;
end **
delimiter ;
...
delimiter **
create procedure sp_datosGrafica()
begin
	select idCuarto as Cuarto, count(*) as Usos from reservacion group by idCuarto;
end **
delimiter ;
...
begin 
    ...
    if (select count(*) from Reservacion where idCuarto = idC and fechaInicio >= fecIni and fechaTermino <=fecFin and esActiva = 1) > 0 then
		set msj = "Reservacion ya activa";
	else
		if(select esReservable from Cuarto where idCuarto = idC) > 0 then 
			set precio = (select precioDiario from Cuarto where idCuarto = idC);
			insert into Reservacion(idHuesped,idCuarto,fechaInicio,fechaTermino,precioTotal,esActiva)
			values(idH,idC,fecIni,fecFin,datediff(fecFin,fecIni)*precio,1);
			set msj = "Reservacion realizada";
        else
...
delimiter **
create procedure sp_cancelarReservacion(idR int)
begin 
	declare msj nvarchar(30);
    declare precio decimal;
...
delimiter **
create procedure sp_consultarReservaciones()
begin
...
delimiter **
create procedure sp_consultarReservacionesHuesped(idH int)
begin
	select * from Reservacion where idHuesped = idH;
end **
delimiter ;

-- POBLACIÓN DE TABLAS
insert into Cuarto(idCuarto,precioDiario)values(1,1200);
insert into Cuarto(idCuarto,precioDiario)values(2,1200);
insert into Cuarto(idCuarto,precioDiario)values(3,1200);
insert into Cuarto(idCuarto,precioDiario)values(4,1200);
insert into Cuarto(idCuarto,precioDiario)values(5,1200);
insert into Cuarto(idCuarto,precioDiario)values(6,1200);
insert into Cuarto(idCuarto,precioDiario)values(7,1200);
insert into Cuarto(idCuarto,precioDiario)values(8,1200);
insert into Cuarto(idCuarto,precioDiario)values(9,1200);
insert into Cuarto(idCuarto,precioDiario)values(10,800);
insert into Cuarto(idCuarto,precioDiario)values(11,800);
insert into Cuarto(idCuarto,precioDiario)values(12,800);
insert into Cuarto(idCuarto,precioDiario)values(13,800);
insert into Cuarto(idCuarto,precioDiario)values(14,800);
insert into Cuarto(idCuarto,precioDiario)values(15,800);
insert into Cuarto(idCuarto,precioDiario)values(16,600);
insert into Cuarto(idCuarto,precioDiario)values(17,600);
insert into Cuarto(idCuarto,precioDiario)values(18,600);
insert into Cuarto(idCuarto,precioDiario)values(19,600);
insert into Cuarto(idCuarto,precioDiario)values(20,600);

insert into Estado(idEstado, clave, nombre, abreviacion, activo) values
